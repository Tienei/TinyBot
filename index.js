        ...
        if (command == bot_prefix + 'help') {
            try {
                function addhelp(helpcommand, fullcommand, description, option, example) {
                    var helptext = '```' + `{prefix}` + fullcommand + '```' + `\n${description}\n\n**---[Options]:**\n${option}\n\n**---[Example]:**\n` + `{prefix}`+ example
                ...
                if (bot_command_help.length < 1) {
                    ...
                    addhelp('rxakatr', 'rxakatr (username)', "Get player's most recent play", 'username: Akatsuki username of the player (Space replaced with "_" or just use quotation mark ``"``)', 'rxakatr Tienei')
                    ...
                    addhelp('calcrxpp', 'calrxcpp (map id) (mods) (acc) (combo) (miss)', "Calculate a play's relax pp (Akatsuki)", '**Needs all options to be calculated**', 'calcrxpp 1157868 nomod 100 1642 0')
                    // Ripple
...
<:rankingA:520932311613571072>: ${user.a}`, true)
                    ...
                    for (var i = 0; i < 50; i++) {
                        ...
                        if (mode == 0) {
                            ...
                            var acc_skill = (Math.pow(thing.star.aim, (Math.pow(best[i].acc, 2.5)/Math.pow(100, 2.5)) * (0.092 * Math.log10(thing.star.nsingles*900000000) * (Math.pow(1.3, best[i].combo/best[i].fc) - 0.3))) + Math.pow(thing.star.speed, (Math.pow(best[i].acc, 2.5)/ Math.pow(100, 2.5)) * (0.099 * Math.log10(thing.star.nsingles*900000000) * (Math.pow(1.35, best[i].combo/best[i].fc) - 0.3)))) * (Math.pow(detail.od, 0.02) / Math.pow(6, 0.02)) * (Math.pow(detail.hp, 0.02) / (Math.pow(6, 0.02)))
                            star_avg += star_skill
                            aim_avg += aim_skill
